...
from hwr.train import TaskPartitionedAutonomousReplayBuffer
from tests.test_autonomous_replay import _episode
...
def test_task_partitioned_replay_balances_scenes_despite_unequal_history() -> None:
    ...
    assert replay.task_sizes() == {"basket": 4, "drawer": 16, "tray": 0}
    assert batch.rewards.shape == (12,)
    assert replay.episode_count == 5
    assert replay.legacy_discarded_hindsight_count == 0


def test_task_partitioned_replay_round_trips_all_partition_state() -> None:
    replay = TaskPartitionedAutonomousReplayBuffer(
        96, ("basket", "drawer", "tray"), seed=4
    )
    replay.add_episode("tray", _episode())
    restored = TaskPartitionedAutonomousReplayBuffer(
...
def test_task_partitioned_replay_can_shrink_during_audited_fork() -> None:
    ...
    assert restored.task_sizes()["tray"] == 12
