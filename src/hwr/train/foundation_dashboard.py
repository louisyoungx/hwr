...
<title>HWR Training Monitor</title><style>
...
<body><h1>HWR Foundation Training Monitor</h1><p id="stamp" class="muted"></p><div id="root"></div>
<script>
...
function render(x){const m=x.metrics||{},t=m.training||m.metrics||{},c=m.action_causality||{};
 const eps=x.episodes||{}, stage=m.stage||m.record_type||'Waiting for metrics';
 const ratio=c.shuffled_to_true_ratio??t['world/action_causality_ratio'];
 const rows=(eps.recent||[]).map(e=>`<tr><td>${e.episode_index}</td><td>${e.task_id}</td><td>${e.action_source}</td><td>${n(e.episode_return)}</td><td>${e.success?'Yes':'No'}</td><td>${n(e.safety_intervention_rate)}</td></tr>`).join('');
 document.getElementById('root').innerHTML=`<div class="grid">${card('Status',stage)}${card('Episode',`${eps.count||0} / ${x.manifest?.training_config?.episodes??'—'}`)}${card('Updates',m.update_count??'—')}${card('Successes',eps.success_count||0,(eps.success_count||0)>0?'ok':'')}${card('Action Causality Ratio',n(ratio),ratio>=1.05?'ok':'bad')}${card('World Model Loss',n(t['world/total']))}</div><div class="card"><h2>Recent Episodes</h2><table><thead><tr><th>#</th><th>Task</th><th>Action Source</th><th>Return</th><th>Success</th><th>Safety Intervention</th></tr></thead><tbody>${rows}</tbody></table></div><div class="card"><h2>Latest Complete Metrics</h2><pre>${JSON.stringify(m,null,2)}</pre></div>`;
 document.getElementById('stamp').textContent=`Run: ${x.run_id} · This page reads small JSON every 5 seconds; replay/weights are not read · ${new Date().toLocaleTimeString()}`;}
