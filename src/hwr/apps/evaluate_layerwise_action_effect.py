"""Run the frozen R0001-P21 layerwise RSSM action-effect diagnostic."""
...
def run(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    if selection_sha256 != EXPECTED_WINDOW_SELECTION:
        raise ValueError("P21 frozen window selection identity differs")
    ...
    try:
        for source, index in selected.items():
            ...
            report = evaluate_layerwise_action_effect(
                trainer.world_model,
                observed.sequence,
                batch.executed_actions,
            )
        ...
        aggregate.update(
            {
                ...
                "invocation": {
                    "module": "hwr.apps.evaluate_layerwise_action_effect",
                    "device": str(arguments.device),
                    "input_run": str(input_run),
                    "checkpoint": str(checkpoint),
                    "output": str(output),
        ...
        _write_json(
            output / "failure.json",
            {
                "schema_version": "hwr.layerwise-action-effect-failure/v1",
                "proposal_id": "R0001-P21",
                "source_commit": source_commit,
                "device": str(arguments.device),
...
def _write_manifest(output: Path, source_commit: str) -> None:
    ...
    _write_json(
        ...
        {
            "schema_version": "hwr.layerwise-action-effect-artifacts/v1",
            "proposal_id": "R0001-P21",
            "source_commit": source_commit,
            "artifacts": {
...
def _source_commit(root: Path) -> str:
    ...
    if len(value) != 40:
        raise RuntimeError("P21 diagnostic requires a Git source commit")
    return value
...
def _require_clean_source(root: Path) -> None:
    ...
    if value:
        raise RuntimeError("P21 diagnostic requires clean committed source")
...
def _require_frozen_invocation(
    ...
    actual = (input_run, checkpoint, output, device)
    if actual != expected:
        raise ValueError("P21 invocation differs from frozen experiment")
