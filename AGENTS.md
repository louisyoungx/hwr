...
- 收到“阅读 AGENTS.md 并启动”或“新一轮主 Agent”类启动指令后，必须在同一回合立即调用工具开始执行，至少完成约束核对、工作区检查和新轮次目录创建；禁止只回复计划、说明、承诺或进度预告后便结束回合。
- Preamble、计划和进度更新不算任务进展，也不是结束回合的理由。发出这类消息后必须继续执行后续工具调用，直到取得可验证的阶段成果，或遇到需要用户输入、权限批准、外部依赖缺失等真实阻塞。
- 不得把“我现在开始”“随后执行”“先读取”等未来式说明当作本轮工作的最终答复。若发生工具错误、意外中断或恢复，必须先检查是否存在部分执行结果，再从未完成步骤继续，不得再次以空转说明结束。
...
2. 生成提案
   ...
   - 合并重复提案，但保留不同意见和来源。
   - 要给子 Agent 充足的排队和任务处理时间，避免主 Agent 过早干预。
...
6. 训练
   ...
   - 只有正式训练才允许休眠，小任务禁止休眠，必须随时关注小任务状态等待任务结束。
   - 休眠期间确保无 Codex 目标，防止被目标自动唤起，尽量多使用 `traex-host-exec` 来替代 Codex 目标。就算要创建目标也禁止设置任何 token 预算。
   ...
   - 所有脚本运行后，会话退出前都要设置定时看门狗，防止脚本运行异常导致会话无法正常唤起。
   - 后台任务（定时看门狗和训练）注意不要直接执行命令，否则会话停止时会被自动 kill，本机有 tmux 可用。
   - 所有自动唤起的 prompt，必须带有要求 “阅读 AGENTS.md”，防止长时间运行后遗忘本任务
