...
训练架构已收敛为[端到端视觉—语言—动作训练范式](./end-to-end-training-paradigm.md)，该文档是当前唯一口径。本节只保留方案摘要。
...
- 先用成功轨迹进行行为克隆预热，再用非对称 actor-critic 强化学习完成闭环训练；
- critic 和奖励可以在训练期使用仿真真值，但 Actor 从训练到部署始终只看可由真机复现的输入；
- 失败轨迹进入 replay buffer 供重采样，不转换成人工规则；
- 自然语言先验来自本地预训练文本编码器，场景语义通过指令、视觉、动作和奖励的配对获得；
- 部署 checkpoint 不包含 critic、专家脚本或仿真特权字段，并必须从磁盘重载后通过隔离种子闭环评测。
