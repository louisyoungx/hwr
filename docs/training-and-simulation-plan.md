...
- 稀有成功、失败和新颖状态按优先级回放；
- 将自主轨迹中的安全单侧接近/接触状态纳入自动 frontier 初始状态池，并持续混合完整任务 reset；
...
- frontier 课程只恢复自主访问过的完整瞬时动力学状态（位置、速度、执行器载荷和不透明求解器状态），不保存未来动作标签，并沿用来源 Episode 的域随机化参数；评测禁用 frontier reset；
