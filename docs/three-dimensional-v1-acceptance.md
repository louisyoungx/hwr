...
| C 三维机器人 | 四轮底盘、6-DOF arm、双指夹爪、头部 RGB-D、腕部 RGB；质量/惯量/碰撞/限位 | 编译后的模型清单和动力学测试 | `verify_robot_model.py` |
...
| F 本机训练 | 本机生成数据、训练、登记、重载；评测只执行学习策略 | 训练 manifest、设备信息、模型哈希、动作来源日志 | `verify_training_lineage.py` |
| G 三任务评测 | 三个指定任务，各 20 个隔离随机种子，成功率 ≥70%，严重碰撞 0 | 逐 Episode 报告和聚合报告 | `verify_3d_benchmarks.py` |
| H 原始证据 | 第三人称 + 头部/腕部视角，不剪接；最终状态稳定 ≥2 秒 | MP4、帧时间线、相机样本、哈希 | `verify_replay_evidence.py` |
...
- 机器人从房间入口出发，绕过茶几或沙发；
- 从相互分离的地面位置拾取至少两个不同 mesh 的玩具；
- 将玩具放入有实际壁厚和碰撞体的收纳箱；
- 必须发生底盘导航、手臂抓取、运输和释放。

### `clear_dining_table_3d/v1`

- 桌面至少包含一个杯子和一个盘子，几何、质量和抓取姿态不同；
- 杯、盘分别进入规定收纳位，不允许只放到同一个大区域；
- 成功时两件物体均在目标容器内并稳定至少 2 秒。

### `store_kitchen_items_3d/v1`

- 至少两类厨房物品进入不同容器或橱柜区域；
- 至少操作一次具有铰链或滑轨关节的柜门/抽屉；
- 柜门/抽屉的运动必须由机器人接触或夹持产生，禁止任务脚本直接设置关节位置。

三个任务都必须使用移动底盘和机械臂。只在工作台原地抓取不算通过。
...
- 四个独立可见轮体和轮地接触碰撞体；
- 可实现差速移动的底盘驱动；
- 六个旋转机械臂关节，逐关节位置、速度、力矩限制；
- 双指夹爪，两个可动夹指、接触面和力限制；
- 头部 RGB 相机与同位深度相机、腕部 RGB 相机；
- 每个动态 link 的正质量、正定惯量和碰撞几何；
- 自碰撞白名单/黑名单和底盘—手臂安全限位。
...
训练数据的真值标签与策略输入分开存储。导出训练张量时执行白名单，任何额外字段导致构建失败。规则专家可以生成示范，但最终 checkpoint 重载后的评测动作 `source` 必须全部是学习策略版本。
