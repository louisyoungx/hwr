...
## 2026-08-14 训练可达性与因果语义修复

外部复核指出的 Replay 容量死锁、留出缓存失控、安全层动作改写缺失和碰撞窗口最大值
验证均已按架构问题处理，没有通过降低成功、安全或物理门槛规避。正式训练仍保持停止。

普通 Replay 现在对每个自主 Episode 只保留两个连续、互不重叠的 16-transition 物理证据
窗口，选择器只读取本体变化、动作变化和安全干预，不读取对象、任务阶段或动作答案。接触、
受控运动和碰撞覆盖按原始 Episode ID 去重；120 个 Episode 最多保留 3,840 个 transition，
18,000 容量不再与每任务 8+8 个碰撞正负 Episode 准入条件冲突。配置构造和开发门禁会静态
证明容量足以保留准入证据。

独立留出集拆成两个时间阶段，但仍共用一个不可变、优化器不可见的存储合同：启动时每任务
采集 8 个 128-transition 系统辨识 Episode，使用 `0.0/0.5/0.9/0.96` 四种任务无关动作
相关度并各选 8 个窗口；探索 Actor 解锁后才补采每任务 8 个严重碰撞正例、8 个负例，每条
只保留末端 16 transition。留出数据只物化语言缓存和原始视觉输入，不再生成 DINOv3 或
SigLIP2 教师特征。运行前资源预检按未压缩上界估算当前正式配置约 `20.84 GiB`，超过
`30 GiB` 配置上限或空闲空间不足 `35 GiB` 时拒绝启动。
...
世界模型新增通用的“当前潜状态 + Actor 提议动作 -> 安全层实际执行动作”残差模型，监督
来自 Replay 中真实的提议/执行动作对。任务 Actor 与内在探索 Actor 的想象轨迹都使用预测
执行动作推进 RSSM，同时惩罚安全干预概率、动作改写幅度和严重碰撞；部署运行时的硬安全
过滤器仍独立于学习模型。动作因果报告与 Actor readiness schema 分别升为 v6，旧 run 不得
进入该谱系。
