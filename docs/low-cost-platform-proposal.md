...
- `TaskEvaluator`：只读环境状态，计算成功、失败、奖励和 achieved goal，不输出动作。
...
4. 从最终状态计算成功、失败、奖励和 achieved goal；
5. 写入 replay buffer，执行目标重标记和非对称 Actor-Critic 更新；
...
- 当前任务文本；
- 人工接管标记；
- Episode 成功、失败、奖励和 achieved goal；
- 标定版本、机器人 ID 和模型版本；
- 急停、过流、通信超时等安全事件。
