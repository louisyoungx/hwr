...
保持观测序列不变，再对所有任务与时间位置上的安全层实际执行动作做无固定点全局置换，
分别计算未来视觉潜变量、本体、奖励、continue/terminal 和安全结果的多步 open-loop
误差。置换保持动作多重集合不变，算法不读取场景对象或任务语义。

正式门槛要求打乱动作后的平均误差至少为真实动作误差的 `1.05` 倍，且至少 `60%` 的
预测 horizon 逐项恶化；全局汇总和按通用 `task_id` 分区的每一项必须同时通过，不能由
一个强任务掩盖另一个失败任务。若没有恶化，说明模型只学习了视频连续性，不能用于
策略想象。
...
最终评测再交叉验证同一份报告及哈希。任何缺失、失败或篡改都使训练/评测返回失败。
