...
世界模型必须通过动作因果反作弊评测：保持观测序列不变，把安全层实际执行的动作沿时间
维置换，再分别计算未来视觉潜变量、本体、奖励、continue/terminal 和安全结果的多步
open-loop 误差。正式门槛要求打乱动作后的平均误差至少为真实动作误差的 `1.05` 倍，且
至少 `60%` 的预测 horizon 逐项恶化。若没有恶化，说明模型只学习了视频连续性，不能
用于策略想象。

每个训练 checkpoint 都必须生成不可变的动作因果报告，报告绑定源码提交、更新次数和
replay manifest SHA-256；报告 SHA-256 同时写入训练 checkpoint。未通过的更新允许保存
训练状态以便诊断，但不得导出部署模型。只有通过报告的同一次更新才能导出部署模型，部署
manifest、`latest.json` 和最终评测再交叉验证同一份报告及哈希。任何缺失、失败或篡改都
使训练/评测返回失败。
